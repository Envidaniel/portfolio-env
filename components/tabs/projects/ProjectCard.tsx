...
import { ExternalLink, FolderGit, Github, ZoomIn } from "lucide-react";
    ...
    >
        <div className="group h-full flex flex-col bg-card-bg p-4 rounded-xl  border border-border-color hover:border-accent/50 transition-all">
            ...
            <div className="flext gap-4 pt-4 border-t border-white/5">
                <a href={project.html_url} target="_blank" className="flex items-center gap-2 text-secondary hover:text-accent"><FolderGit size={14} />Code</a>
                {project.homepage && <a href={project.homepage} target="_blank" className="flex items-center gap-2 text-xs text-secondary hover:text-accent"><ExternalLink size={14} />Demo</a>}
            </div>
